 ...
 * {@see \Northrook\Contracts\ContainerInterface::DEFAULT_REFERENCE} key carries
 * primary constructor/factory overrides (prefer
 * {@see \Northrook\Contracts\ServiceDefinition::setArguments()}).
 *
 ...
 * {@see \Northrook\Contracts\Container\CompilerPass::VALIDATE}: named keys may
 * omit parameters; positional keys must align with the signature.
 *
 * Intended end state: `Tag.reference` aligns with
 * {@see \Northrook\Contracts\ContainerInterface::get()} `$reference` for named bindings.
 *
...
final readonly class Tag implements AutodiscoverInterface
{
     ...
     * signature (validated at {@see \Northrook\Contracts\Container\CompilerPass::VALIDATE}).
     ...
     *
     *
     * @return \Northrook\Contracts\Service\Tag
     */
    public static function from(
        ...
        if (empty($value)) {
            throw new InvalidArgumentException(
                ...
                received: $value,
                context : ['value' => $value, 'arguments' => $arguments],
            );
        }
