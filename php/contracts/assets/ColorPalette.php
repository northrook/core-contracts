 ...
 */
final readonly class ColorPalette extends DataObject implements Exportable
{
    /**
     ...
     *
     * Prefix and name are concatenated as-is after stripping a single leading
     * `-` from each — no separator is inserted. Pass the separator in `$prefix`
     * when you want one (e.g. `theme-` → `--theme-background`; `theme` →
     * `--themebackground`).
     *
     ...
     * With `$prefix = 'theme-'` → `var(--theme-background)`; `$prefix = 'theme'`
     * → `var(--themebackground)` (no auto-inserted hyphen).
     *
    ...
    private function variableName(
        ...
        // Literal concat; no hyphen injected between prefix and name.
