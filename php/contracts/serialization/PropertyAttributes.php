...
use Northrook\Parameter\Secret as SecretTier;
...
final class PropertyAttributes implements Resettable
{
    ...
    /**
     * Combined secrecy tier + tag seeds from every {@see Secret} / {@see \SensitiveParameter}.
     *
     * Multiple hits merge defensively:
     * - tier: {@see SecretTier::CREDENTIAL} wins over {@see SecretTier::SENSITIVE}
     * - tags: union (order of first appearance) from attribute `$conditions` and
     *   `\SensitiveParameter::class` when that attribute is present
     *
     ...
     * @param \ReflectionClass<covariant object> $class
     *
     * @return null|array{secret: SecretTier, tags: list<non-empty-string>}
     */
    public static function redaction(
        ...
        $tier = null;
        /** @var array<non-empty-string, non-empty-string> $tags */
        $tags = [];
        ...
        foreach (self::resolve($class, $property) as $attribute) {
            ...
            if ($name === Secret::class) {
                ...
                $tier =
                    $tier === SecretTier::CREDENTIAL || $policy === SecretTier::CREDENTIAL
                        ? SecretTier::CREDENTIAL
                        : SecretTier::SENSITIVE;
            ...
            if ($name === \SensitiveParameter::class) {
                $tier                             ??= SecretTier::SENSITIVE;
                $tags[\SensitiveParameter::class] ??= \SensitiveParameter::class;
            }
        }
